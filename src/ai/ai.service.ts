...
export class AiService implements OnModuleInit {
  ...
  async analyzeInputDuringDataCollection(prompt: string, currentTemplateName: string): Promise<{ intent: 'provide_data' | 'switch_document' | 'general_query'| 'repeat_request'; templateName?: string }> {
    ...
    // `;
    const intentAnalysisPrompt = `
        Ты - анализатор намерений. Пользователь СЕЙЧАС заполняет документ "${currentTemplateName}".
        Определи ТОЧНОЕ намерение из ВОЗМОЖНЫХ ВАРИАНТОВ:

        ### КРИТЕРИИ ДЛЯ КАЖДОГО НАМЕРЕНИЯ:
        1. "provide_data" - ТОЛЬКО если текст:
           - Является прямым ответом на вопросы по текущему документу
           - Содержит конкретные данные (даты, имена, числа)
           - Имеет структуру "поле: значение"

        2. "switch_document" - ТОЛЬКО если:
           - Четко указано название другого документа из списка
           - Есть фразы типа "хочу другой документ", "смените на..."
           - И вернуть ТОЧНОЕ имя файла шаблона

        3. "repeat_request" - ТОЛЬКО если:
           - Текст почти идентичен начальному запросу на этот документ
           - Содержит фразы "повтори", "еще раз", "не понял"

        4. "general_query" - ВСЕ остальные случаи:
           - Вопросы не по теме
           - Жалобы (как "залило соседи")
           - Команды отмены
           - Любые сообщения без данных

        ### СТРОГИЕ ПРАВИЛА:
        - НИКОГДА не предлагай новый документ, если пользователь не указал его ЯВНО
        - При малейшем сомнении - выбирай "general_query"
        - Для "switch_document" ВСЕГДА проверяй наличие шаблона

        Список доступных документов:
        ${this._templateNames.slice(0, 5).map(t => `- ${t.humanName} (${t.fileName})`).join('\n')}
        ${this._templateNames.length > 5 ? `\n(и еще ${this._templateNames.length - 5} документов)` : ''}

        Текущий документ: ${currentTemplateName}
        Запрос: "${prompt}"

        Верни ТОЛЬКО JSON без пояснений. Примеры:
        {"intent": "provide_data"}
        {"intent": "general_query"}
        {"intent": "switch_document", "templateName": "akt.docx"}
    `;
