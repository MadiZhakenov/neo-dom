  ...
  private async getFactualAnswer(prompt: string, history: Content[], language: 'ru' | 'kz'): Promise<string> {
    ...
    const finalPrompt = `
      ...
      **ТВОЙ ГЛАВНЫЙ ПРИОРИТЕТ - ИСТОРИЯ ЧАТА.**
      ...
      6. Проанализируй "Вопрос пользователя". **Самостоятельно определи основной язык вопроса** (казахский или русский), даже если он написан латиницей или смешанно ("шала-казахский").
      7. **Твой ответ ВСЕГДА должен быть на том языке, на котором задан вопрос.** Моя предварительная оценка языка - (${language}), но доверяй своему анализу. МАКСИМАЛЬНО АНАЛИЗИРУЙ НАПИСАННЫЙ ПОЛЬЗОВАТЕЛЕМ ТЕКСТ И ПЫТАЙСЯ ПОНЯТЬ ЯЗЫК СООБЩЕНИЯ. Если пользователь просит "давай на казахском", ты ОБЯЗАН переключиться.
      8. **Сначала** попробуй найти ответ в предоставленном "Контексте из документов". Если вопрос касается правил, законов или требует точных данных — это твой главный источник.
      9. **Если в документах нет ответа**, не говори "Я не знаю". Ответь на вопрос, используя "Историю чата" (если это уместно, например, "какой был мой последний вопрос?") или свои общие знания по теме ЖКХ и ОСИ.
      10. Если вопрос общий (например, "как дела?"), просто поддерживай дружелюбный разговор.
