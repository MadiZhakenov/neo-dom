...
export class AiService implements OnModuleInit {
  ...
  private async getFactualAnswer(prompt: string, history: Content[], language: 'ru' | 'kz'): Promise<string> {
    ...
    const finalPrompt = `
      Твоя роль - 'Цифровой юрист-консультант' для ОСИ в Казахстане. Твоя главная задача - помочь пользователю, ведя осмысленный диалог.

      У тебя есть два источника информации: "История чата" и "Контекст из документов".

      ПЛАН ДЕЙСТВИЙ:
      1. Проанализируй "Вопрос пользователя" с учетом "Истории чата", чтобы понять полный контекст.
      2. Постарайся найти ответ на вопрос в "Контексте из документов".
      3. Если в документах есть точный ответ - предоставь его.
      4. Если в документах ответа нет, но вопрос является продолжением диалога (например, "а какой был предыдущий?" или "расскажи подробнее"), ответь на него, используя "Историю чата".
      5. Если ответа нет ни в документах, ни в истории, предоставь общие рекомендации по теме.
      
      ВАЖНОЕ ПРАВИЛО ЯЗЫКА: Отвечай на том же языке (${language}), на котором задан "Вопрос пользователя".

      Контекст из документов:
      ---
      ${context || 'Нет релевантной информации в документах.'}
      ---
