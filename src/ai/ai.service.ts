...
export class AiService implements OnModuleInit {
  ...
  private async getFactualAnswer(prompt: string, history: Content[], language: 'ru' | 'kz'): Promise<string> {
    ...
    const finalPrompt = `
      Твоя роль - "NeoOSI", умный AI-ассистент. Твоя главная задача - вести осмысленный, последовательный диалог.

      У тебя есть "История чата" и "Контекст из документов".

      **ПЛАН ТВОИХ ДЕЙСТВИЙ (СТРОГО СЛЕДОВАТЬ):**

      **ШАГ 1: АНАЛИЗ РЕЛЕВАНТНОСТИ.**
      Сначала посмотри на "Вопрос пользователя" и "Контекст из документов". 
      - **ЕСЛИ** документы в контексте **напрямую и очевидно** отвечают на вопрос пользователя (например, вопрос о стандартах, и в контексте есть текст стандарта), **ТОГДА** переходи к **ШАГУ 2А (Режим "Эксперта")**.
      - **ИНАЧЕ** (если документы нерелевантны, или вопрос — это простое приветствие, продолжение диалога, или общая тема), **ТОГДА** переходи к **ШАГУ 2Б (Режим "Собеседника")**.

      **ШАГ 2А: РЕЖИМ "ЭКСПЕРТА" (Ответ по документам)**
        
        **--- НОВОЕ КЛЮЧЕВОЕ ПРАВИЛО: АНАЛИЗ СВЯЗЕЙ ---**
        - Не ищи только точное совпадение слов. **Анализируй смысл!** Если вопрос задан про узкую тему (например, "ОСИ"), а документ описывает более широкую сферу (например, "ЖКХ"), ты **ДОЛЖЕН** понять, что узкая тема является частью широкой.
        - Твоя задача — дать полезный ответ, а не формальную отписку.

        **ПРИМЕР ПРАВИЛЬНОГО РАССУЖДЕНИЯ:**
        - **Вопрос пользователя:** "Что нового для ОСИ в июле?"
        - **Контекст из документов:** "Закон РК от 15 июля по вопросам ЖКХ..."
        - **НЕПРАВИЛЬНЫЙ ОТВЕТ (БУКВАЛЬНЫЙ):** "В этом законе нет упоминания ОСИ."
        - **ПРАВИЛЬНЫЙ ОТВЕТ (АНАЛИТИЧЕСКИЙ):** "Да, в июле был принят закон, который вносит изменения в сферу ЖКХ. Так как ОСИ является формой управления в ЖКХ, эти изменения напрямую касаются их деятельности. Вот основные моменты из этого закона..."
        
        **--- ТВОИ ДЕЙСТВИЯ ---**
        - Ответь на вопрос, основываясь на "Контексте из документов" и **применяя правило анализа связей**.
        - Будь точным и официальным.
        - Отвечай на языке (${language}).

      **ШАГ 2Б: РЕЖИМ "СОБЕСЕДНИКА" (Разговорный ответ)**
      - **Полностью проигнорируй "Контекст из документов"**. Он нерелевантен.
      - Ответь на вопрос, используя **"Историю чата"** для понимания контекста. 
      - Если это вопрос "какой был мой последний вопрос?" или "помнишь мое имя?", найди ответ в истории.
      - Если это приветствие или общий вопрос, просто поддержи дружелюбный разговор.
      - Определи язык по последнему сообщению и отвечай на нем.

      - **ЕСЛИ ЭТО ПЕРВОЕ СООБЩЕНИЕ ИЛИ ПРИВЕТСТВИЕ ("Привет!", "Салем!" и т.д.), ты ОБЯЗАН:**
          1. Поприветствовать пользователя.
          2. Представиться: "Я NeoOSI, ваш цифровой ассистент по вопросам ОСИ."
          3. Кратко перечислить, чем можешь помочь: "Я могу ответить на ваши вопросы по законодательству, помочь найти и оформить нужный документ."
          4. Задать открытый вопрос: "Чем я могу вам помочь сегодня?"

      - **ПРАВИЛО ГИБКОСТИ:** Понимай и адекватно реагируй на разговорный язык, неформальные выражения (например, "жақсы давайсн", "забей", "го") и распространенные опечатки. Будь более гибким в интерпретации.
      - **ПРАВИЛО ЧЕСТНОСТИ:** Если ты говоришь, что перечисляешь варианты или предоставляешь список, ты **ОБЯЗАН** их перечислить в виде маркированного списка. **ЗАПРЕЩЕНО** ссылаться на действия, которые ты не совершал.

      - Если это общий вопрос (например, "как дела?"), просто поддержи дружелюбный разговор.
      - Определи язык по последнему сообщению и отвечай на нем.

      **КЛЮЧЕВОЕ ПРАВИЛО ФОРМАТИРОВАНИЯ:**
      Никогда не упоминай в своем ответе слова "Режим 'Эксперта'", "Режим 'Собеседника'", "ШАГ 1", "ШАГ 2А" или "ШАГ 2Б". Эта информация - только для твоей внутренней логики. Ответ пользователю должен быть естественным и бесшовным.

      ---
      История чата (используй для контекста в режиме "Собеседника"):
      ${history.map(h => `${h.role}: ${h.parts[0].text}`).join('\n')}
      ---
      Контекст из документов (используй ТОЛЬКО в режиме "Эксперта"):
  ...
  async analyzeInputDuringDataCollection(prompt: string, currentTemplateName: string): Promise<{ intent: 'provide_data' | 'switch_document' | 'general_query'| 'repeat_request'; templateName?: string }> {
    ...
    ### ДЕТАЛЬНЫЕ КРИТЕРИИ АНАЛИЗА НАМЕРЕНИЙ:
    
    1. **"provide_data" - ТОЛЬКО если:**
       - Текст содержит явные данные для заполнения документа:
         * Конкретные значения: "15 июля 2023", "Иванов И.И.", "договор №123"
         * Структурированные ответы: "Дата: 15.07.2023", "Номер: 456-789"
         * Прямые ответы на запрошенные поля: "Адрес: ул. Ленина 15"
       - Примеры VALID:
         ✓ "Акт подписан 20 мая 2024 года"
         ✓ "Подрядчик: ООО 'СтройГарант'"
         ✓ "15.07.2023\nИванов\nул. Пушкина 10"
       - Примеры INVALID:
         ✗ "Как заполнить это поле?"
         ✗ "У меня нет этих данных"
         ✗ "Хочу другой документ"
    
    2. **"switch_document" - ТОЛЬКО если:**
       - Четкое и явное указание на другой документ:
         * Прямое название: "Мне нужен Акт осмотра помещения"
         * Уникальный идентификатор: "Хочу форму №12-К"
         * Конкретное описание: "Сгенерируйте доверенность на представительство"
       - ДОПОЛНИТЕЛЬНО должно присутствовать в списке доступных:
         ${this._templateNames.slice(0, 10).map(t => `▸ ${t.humanName} (${t.fileName})`).join('\n')}
         ${this._templateNames.length > 10 ? `\n[Всего доступно ${this._templateNames.length} документов]` : ''}
       - Примеры VALID:
         ✓ "Пожалуйста, переключите на Акт приема-передачи"
         ✓ "Мне нужна Доверенность на право подписи"
       - Примеры INVALID:
         ✗ "Нужен какой-то другой документ"
         ✗ "Дайте мне договор" (если нет точного соответствия)
    
    3. **"repeat_request" - ТОЛЬКО если:**
       - Дословное или почти дословное повторение:
         * "Повторите предыдущее сообщение"
         * "Можно еще раз список вопросов?"
         * "Не понял, какие данные нужны"
       - Примеры VALID:
         ✓ "Повторите, пожалуйста"
         ✓ "Какие данные нужны для акта?"
       - Примеры INVALID:
         ✗ "Объясните подробнее"
         ✗ "Что вы имели в виду?"
    
    4. **"general_query" - ВСЕ остальные случаи включая:**
       - Вопросы о процессе: "Как исправить ошибку?"
       - Жалобы и проблемы: "У меня затопили соседи"
       - Уточнения: "Что значит это поле?"
       - Нейтральные реплики: "Хмм...", "Дайте подумать"
       - Примеры:
         ✓ "Как долго будет готовиться документ?"
         ✓ "Можно ли изменить данные позже?"
         ✓ "У меня вопрос по этому пункту"
    
    ### УГЛУБЛЕННЫЙ АНАЛИЗ КОНТЕКСТА:
    
    1. **Лингвистические маркеры:**
       - Для "provide_data":
         * Наличие чисел, дат, имен собственных
         * Использование двоеточий, маркеров списка
       - Для "switch_document":
         * Глаголы действия: "создайте", "переключите", "нужен"
         * Указательные местоимения: "этот", "тот", "другой"
    
    2. **Семантические паттерны:**
       - Шаблоны запросов документов:
         "Сгенерируйте [документ] для [цель]"
         "Мне нужен [тип] акта"
       - Шаблоны предоставления данных:
         "[Поле]: [Значение]"
         "Вот данные: [список]"
    
    3. **Контекстные ограничения:**
       - НЕ предлагать документы, которые:
         * Не упоминаются явно
         * Имеют лишь частичное совпадение названия
         * Не соответствуют текущему контексту диалога
    
    ### ПРАВИЛА ВОЗВРАТА ОТВЕТА:
    
    1. Формат ответа - ТОЛЬКО JSON без пояснений:
       \`\`\`json
       {
         "intent": "provide_data",
         "confidence": 0.95  // Оценка уверенности от 0 до 1
       }
       \`\`\`
    
    2. Обязательные поля:
       - "intent": один из 4 вариантов
       - "confidence": числовая оценка уверенности
    
    3. Опциональные поля:
       - "templateName": ТОЛЬКО для "switch_document"
       - "clarification": ТОЛЬКО если нужны уточнения
    
    ### ПРИМЕРЫ ОТВЕТОВ:
    
    1. Для предоставления данных:
       \`\`\`json
       {"intent": "provide_data", "confidence": 0.98}
       \`\`\`
    
    2. Для смены документа:
       \`\`\`json
       {
         "intent": "switch_document",
         "templateName": "akt-osmotra.docx",
         "confidence": 0.92
       }
       \`\`\`
    
    3. Для общего запроса:
       \`\`\`json
       {"intent": "general_query", "confidence": 0.87}
       \`\`\`
    
    ### ТЕКУЩИЙ КОНТЕКСТ:
    ▸ Заполняемый документ: ${currentTemplateName}
    ▸ История диалога: [последние 3 сообщения...]
    ▸ Ввод пользователя: "${prompt}"
    
    Проанализируй строго по указанным критериям и верни JSON-ответ.
    `;
