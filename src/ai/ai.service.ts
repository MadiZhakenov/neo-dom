...
export class AiService implements OnModuleInit {
  ...
  private async getFactualAnswer(prompt: string, history: Content[], language: 'ru' | 'kz'): Promise<string> {
    ...
    const finalPrompt = `
      ...
      3. **Твой ответ ВСЕГДА должен быть на том языке, на котором задан вопрос.** Моя предварительная оценка языка - (${language}), но доверяй своему анализу. Если пользователь просит "давай на казахском", ты ОБЯЗАН переключиться.
